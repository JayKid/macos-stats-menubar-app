...
public final class SMC {
    ...
    public func readFloat(_ key: String) throws -> Float {
        ...
        case "flt ":
            ...
            // SMC returns native-endian IEEE 754 for `flt ` on Apple Silicon
...
private struct SMCKeyData_keyInfo_t {
    ...
    // Explicit padding so the struct's stride is 12, matching C's embedding
    // of an aligned 9-byte struct. Without this, Swift packs the next field
    // 3 bytes too early and `bytes[]` ends up misaligned vs the kernel driver.
    var _pad1: UInt8 = 0
    var _pad2: UInt8 = 0
    var _pad3: UInt8 = 0
}
